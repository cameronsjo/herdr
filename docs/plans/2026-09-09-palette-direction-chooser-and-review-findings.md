...
- **cadence-forge:security-reviewer** (opus, U9) — 0 Critical, 2 Important, 4 Nits.
  ...
  - **Declined:** a whole-file config parse error drops the override list (the live-reload path already guards it); a long plugin name can push the `[destructive]` tag off the row (**reversed — see the CodeRabbit round below**); the restored palette selection is briefly stale until the plugin list arrives (self-correcting).

### Task 6 — CodeRabbit round (2026-09-10, answered 2026-09-11)

CodeRabbit reviewed the PR after the two dispatched reviews. Two Minor findings, both confirmed against live behavior and both fixed.

- **Fixed, and it reverses a decline above:** the `[destructive]` tag was drawn at the full display width of the row name, but `put_text` clips the name to `rect.width` (`render.rs:351`). A name filling the row made the tag's width argument `0`, so the tag was **not drawn at all** — not merely pushed right, which is what the earlier nit was declined on. The plugin chooses that name, so this is the same front-loading concern the chooser title already defends against. The right column is now resolved into a `(text, style)` pair before the name is drawn, and both it and the tag are reserved out of the name's budget; the name is clipped instead. Reserving only the tag (CodeRabbit's proposed diff) was insufficient — `put_right_text` draws last and overwrote the tag's tail. Pinned by `a_row_name_too_wide_for_the_row_cannot_crowd_out_the_destructive_tag` (`5edab16f`).
- **Fixed:** `scripts/palette-live-check.sh` wrote the config override to `$XDG_CONFIG_HOME/config.toml`, but a debug build reads `$XDG_CONFIG_HOME/herdr-dev/config.toml` (`app_dir_name()`, `src/config/io.rs:23`). Step 5 therefore exercised nothing and would have shown no tag whether the feature worked or not. Beyond the path, the script now asks the binary where it will look (`herdr --help` prints the resolved path) and refuses when the two disagree (`2859f74d`).
...
- **A "cosmetic" decline can be hiding a stronger claim under the same symptom.** The security nit "a long plugin name can push the tag toward the right column" was declined as cosmetic. It shared a root cause with a defect that removes the tag entirely, and the declined sentence described only the mild half. The tell available at decline time: the string that triggers the symptom is chosen by the untrusted party. When that is true, work out the worst value it can take before declining on the value you happened to be looking at.
- **The manual check was the least-verified artifact on the branch.** Every code path had a test; the script that tells the operator what to look at had nothing, and it wrote its config one directory above where the binary reads. Its step 5 would have shown no `[destructive]` tag whether the feature worked or not — a check that cannot go red, on the one gate that exists because a code test cannot cover it. It now asserts its own override path against the binary.
