...
| Screen-assisted lifecycle | Codex | Hook events report state and native session identity. Strong, newer visible screen evidence corrects gaps caused by concurrent stop hooks and permission flows. |
| Session identity | Claude Code, GitHub Copilot CLI, Devin CLI, Droid, Qoder CLI, Qwen Code, Cursor Agent CLI, Hermes Agent, Antigravity CLI, Grok CLI | The integration reports native session references for restore. State still comes from Herdr's screen manifest detection. |
...
The Codex hook reports session identity and lifecycle state through the local Herdr socket. `UserPromptSubmit`, `PreToolUse`, and `PostToolUse` report `working`; `PermissionRequest` reports `blocked`; and `Stop` reports `idle`. Codex runs hook handlers concurrently and does not emit an approval-resolved hook, so Herdr keeps screen detection active: a newer strong visible-working signal can correct a stale `Stop` or permission report, and a visible blocker can correct a stale working report.

The hook also reports a presentation title without changing Codex's native thread id. It reads the thread's `/rename` name or preview through Codex's public app-server protocol, then falls back to the submitted prompt when that metadata is unavailable.
