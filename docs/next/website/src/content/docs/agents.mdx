...
| Codex | screen-assisted lifecycle hooks when installed; otherwise screen manifest | state, session, and title |
...
Codex uses a hybrid of those paths when its integration is installed. Hooks report prompt, tool, permission, and stop transitions, while strong visible screen evidence remains active to correct races from concurrent stop hooks and permission flows that have no approval-resolved event. The newer signal wins within those narrow corrections; Codex does not become a complete lifecycle authority.
...
Integrations marked only `session` in the table above are intentionally not lifecycle authorities. They provide native session identity for restore, but their hooks do not cover the whole lifecycle. They can miss permission approval results, escape interrupts, or other transitions. For those agents, Herdr still uses screen manifest detection.
