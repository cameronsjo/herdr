...
that policy without rewriting config. It also suspends
`ui.sidebar.agents.group_by` workspace grouping while the view is active, since
a custom sort can interleave workspaces under one header.
