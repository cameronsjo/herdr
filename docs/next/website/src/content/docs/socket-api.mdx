...
| Workspace | `workspace.create`, `workspace.open`, `workspace.list`, `workspace.get`, `workspace.focus`, `workspace.rename`, `workspace.move`, `workspace.move_block`, `workspace.merge`, `workspace.report_metadata`, `workspace.close` |
| Worktree | `worktree.list`, `worktree.create`, `worktree.open`, `worktree.remove` |
| Tab | `tab.create`, `tab.list`, `tab.get`, `tab.focus`, `tab.rename`, `tab.move`, `tab.move_to_destination`, `tab.close` |
...
| Agent | `agent.list`, `agent.get`, `agent.read`, `agent.explain`, `agent.send_keys`, `agent.type_submit`, `agent.prompt`, `agent.wait`, `agent.rename`, `agent.focus`, `agent.start`, `agent.view.set`, `agent.view.clear` |
...
that policy without rewriting config. A custom sort that interleaves workspaces
suspends `group_by = "workspace"` while the view is active, since one header
would label agents from another workspace. Token grouping
(`group_by = { token = "name" }`) stays on while `ui.agent_panel_sort = "spaces"`:
it gathers each group's agents together and keeps the view's order inside each
group. Under `"priority"`, grouping is off either way.
...
In the TUI, an active view shows its label with a `✕` at the right of the Agents
header. Clicking it, choosing `clear agent view` in the global menu or the
command palette, or pressing `keys.clear_agent_view` (unset by default) sends an
unconditional `agent.view.clear`.
...
### Moving tabs between workspaces (fork)

`tab.move` with `tab_id` and `insert_index` reorders within a workspace and
returns `tab_list`, matching upstream clients. Cross-workspace moves use the
separately advertised `tab.move_to_destination` method with `tab_id` and a
required `destination`: `{ "type": "workspace", "workspace_id": "w2" }` or
`{ "type": "new_workspace" }`. It returns `tab_move` with `move_result` and
`tabs`. The older fork `tab.move` destination form remains accepted.

The client checks the selected server's advertised methods before offering a
request that depends on destination support. Moves and workspace merges stay
within that server; IDs on another machine are not valid destinations.
