...
`SKILL.md` is self-sufficient on its own. It is also accompanied by `references/` files covering CLI semantics, agent lifecycle states, and reading pane output, which agents that support progressive disclosure load only when they need that detail.
...
To install the release-matched copy straight from the binary — `SKILL.md` and its `references/` together — point Herdr at a skill directory:

```bash
herdr --skill --install ~/.claude/skills/herdr
```

This writes the whole directory, so the installed skill always matches the running Herdr build.
