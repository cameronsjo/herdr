...
herdr workspace open [--cwd PATH] [--label TEXT] [--env KEY=VALUE] [--focus] [--no-focus]
...
`open` is the "work on this path" command: it resolves to the workspace already on that path instead of opening a second one there, and creates a workspace only when the path has none. Its response is `.result.workspace` plus `.result.already_open`, which says which of the two happened. `--focus` decides whether the workspace becomes the focused one, for a reused workspace as well as a newly created one; without it, `open` resolves or creates the workspace and leaves your focus where it is. `--label` applies only to a workspace `open` creates: reusing an existing workspace never renames it, so `--label` is not a way to rename one. Use `herdr workspace rename` for that. `create` always creates, so it remains the way to ask for a second workspace on a path you already have open. The TUI new-space action uses `open` unless you type a name for the new space, and always focuses.
...
herdr agent type-submit <target> <text>
...
A successful start returns only after the expected agent owns the same terminal and is ready for interactive input. If detection reports `blocked` during startup, the command returns `agent_not_ready` immediately. The name remains available for `agent read`, `agent send-keys`, and `agent type-submit`, and becomes ready for prompts after detection reports `idle`. The default startup timeout is 30000 milliseconds; explicit values must be greater than 3000 and no more than 300000.
...
`agent send-keys` sends logical terminal keys such as `enter`, `up`, `esc`, or `ctrl+c`. `agent type-submit` sends literal text plus Enter as one interactive UI submission; use it for slash commands or other harness commands that must be typed into the agent UI instead of sent as model prompts. Herdr validates every key or type-submit text before writing any bytes. `agent read` reads the resolved terminal stream, and `agent rename` names an already detected agent.
