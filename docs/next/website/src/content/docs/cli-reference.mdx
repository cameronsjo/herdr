...
`open` is the "work on this path" command: it focuses the workspace already on that path instead of opening a second one there, and creates a workspace only when the path has none. Its response is `.result.workspace` plus `.result.already_open`, which says which of the two happened. `create` always creates, so it remains the way to ask for a second workspace on a path you already have open. The TUI new-space action uses `open` unless you type a name for the new space.
