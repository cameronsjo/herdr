...
`row_gap` は、Agent パネルと Space パネルごとに、エントリ間の空白行数を指定します。デフォルトは `0` で、エントリを詰めて表示します。以前の間隔に戻すには `1` に設定します。`rows` で定義したコンテンツ行の間隔には影響しません。連続するインデントされた worktree の子は、1 つの Space グループとして詰めて表示されます。`group_by` が Agent パネルをグループ化しているあいだ、`row_gap` はワークスペースのまとまりどうしのあいだにだけ適用されます。同じまとまり内のエージェントは、共通の見出しの下に詰めて表示されます。

#### Agent の行をワークスペースでグループ化する

`group_by = "workspace"` を設定すると、すべての Agent エントリでワークスペース名を繰り返す代わりに、同じワークスペースの隣接するエージェントのまとまりごとに 1 つのワークスペース見出しを描画します:
...
グループ化は `rows` の代わりに `grouped_rows` を選択するだけで、設定した行を書き換えることはありません。見出しがすでにワークスペース名を示すため、`grouped_rows` には通常 `workspace` トークンを含めないレイアウトを指定します。グループ化が有効な間は `grouped_rows` が `rows_by_agent` より優先されます。エージェントごとの上書きはグループ化がオフのときに適用されます。

グループ化が有効になるのは、Agent パネルが **grouped**（スペース）順のときだけです。**priority** 順や、ワークスペースを混在させる並び順のエージェントビューでは無効になります。見出しが別のワークスペースのエージェントを指してしまうためです。絞り込みのみのビューはスペース順を保つので、グループ化は有効なままです。無効なときの描画は `group_by = "none"` とまったく同じです。まとまりの途中までスクロールすると、そのまとまりの見出しも画面外へスクロールします。Herdr は見出しを固定しません。
...
上書きは `rows` を置き換えるもので、拡張するものではありません。上書きのキーには、`claude`、`codex`、`pi` のような大文字と小文字を区別する正規エージェント ID を使います。`claude-code` のような検出エイリアスは使えません。カスタム報告されたエージェントを含め、上書きがないエージェントには `rows` が使われます。`group_by` がエントリをグループ化しているあいだは、上書きのあるエージェントも含め、すべてのエージェントに `grouped_rows` が使われます。
