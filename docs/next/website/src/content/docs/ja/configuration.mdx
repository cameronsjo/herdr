...
グループ化が有効になるのは、Agent パネルが **grouped**（スペース）順のときだけです。**priority** 順や、並べ替えを行うエージェントビューでは無効になります。どちらもワークスペースを混在させ、見出しが別のワークスペースのエージェントを指してしまうためです。無効なときの描画は `group_by = "none"` とまったく同じです。まとまりの途中までスクロールすると、そのまとまりの見出しも画面外へスクロールします。Herdr は見出しを固定しません。
