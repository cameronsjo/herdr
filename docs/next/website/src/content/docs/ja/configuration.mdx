...
グループ化は `rows` の代わりに `grouped_rows` を選択するだけで、設定した行を書き換えることはありません。見出しがすでにワークスペース名を示すため、`grouped_rows` には通常 `workspace` トークンを含めないレイアウトを指定します。`rows_by_agent` の上書きは、どちらのモードでも優先されます。
