...
| エージェント | `agent.list`、`agent.get`、`agent.read`、`agent.explain`、`agent.send_keys`、`agent.type_submit`、`agent.prompt`、`agent.wait`、`agent.rename`、`agent.focus`、`agent.start` |
...
そのポリシーを置き換えます。カスタムソートはワークスペースを混在させることがあるため、
ビューが有効なあいだは `ui.sidebar.agents.group_by` のグループ化も停止します。
...
タブのイベント購読には `tab.created`、`tab.closed`、`tab.focused`、`tab.renamed`、`tab.moved`、`tab.moved_across_workspaces` があります。`tab.moved` は、移動した `tab_id`、`workspace_id`、要求された `insert_index`、そのワークスペースの更新済みの順序付き `tabs` リストを含みます。タブがワークスペースをまたいで移動した場合は、代わりに `tab.moved_across_workspaces` が発火し、再発行された `tab_id`、移動元での `previous_tab_id`、`workspace_id`、`source_workspace_id`、`insert_index`、および両方のワークスペースの順序付き `tabs` と `source_tabs` リストを含みます。
