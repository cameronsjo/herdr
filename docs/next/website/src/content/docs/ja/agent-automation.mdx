...
| エージェント UI コマンドを入力して送信する | `agent type-submit` |
| エージェントのライフサイクル状態を待つ | `agent wait` |

`agent prompt` はテキストを送信し、短い遅延の後にエンコード済み Enter を送信します。ターミナルの現在の bracketed paste モードを尊重します。working 中のエージェントにも送信できます。エージェントがすでに `blocked` の場合は、ターミナル入力を送信せずに `agent_blocked` を返します。ダイアログを確認し、意図した応答には `agent send-keys` を使ってください。モデルプロンプトではなくエージェント UI に入力する必要があるスラッシュコマンドやハーネスコマンドには `agent type-submit` を使います。`esc`、`up`、`enter`、`ctrl+c` のような操作には `agent send-keys` を使います。`escape` も `esc` のエイリアスとして使えます。意図的に生のターミナルを操作したい場合は、ペイン入力コマンドを使ってください。
