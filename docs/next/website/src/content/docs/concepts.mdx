...
Moving and reordering are mouse work too. Right-click a workspace, tab, or pane for move and reorder entries; picking `Move to space...` opens the search picker, so you never need to know an id. Drag a tab along the tab bar to reorder it, or onto a workspace in the sidebar to move the whole tab to that space. Drag an agent row in the sidebar onto a workspace to move its pane into a new tab there, or onto a tab to split it against that tab; Herdr asks which way the split goes.
