...
in different workspaces. An agent without the token, or with an empty value,
keeps its workspace group. The name may be written with or without the leading
`$`.
...
order. Metadata tokens can expire (`--ttl-ms`); an agent whose token expired
returns to its workspace group.
