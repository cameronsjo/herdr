...
rows = [
  ...
  ["terminal_title_stripped", { token = "state_text", align = "right" }],
]
```

`fg` accepts strict `#RGB` or `#RRGGBB`. `bold` and `dim` accept booleans. `align = "right"` starts a right-aligned trailing group, so every token from that occurrence through the end of its row stays against the sidebar's right edge. Omitted fields preserve the token's contextual style; explicit `false` removes that modifier. Styling applies to one occurrence, so the same token may look different in another row or agent override. A foreground override replaces all semantic foregrounds inside that occurrence: for example, styled `git_status` ahead and behind counts use one color instead of their default green and red. Token styles never change separators or row backgrounds.
...
`row_gap` controls the blank terminal rows between entries, independently for the Agent and Space panels. It defaults to `0`, which packs entries together; set it to `1` to restore the previous spacing. It does not add spacing between the content lines declared in `rows`. Consecutive indented worktree children remain packed as one Space group. While `group_by` groups the Agent panel, `row_gap` applies only between workspace runs; agents within one run stay packed under their shared header.

#### Grouping Agent rows by workspace

Set `group_by = "workspace"` to draw one workspace header above each run of
adjacent agents from that workspace, instead of repeating the workspace name on
every agent entry:
...
```text
cadence-ecosystem
  ○ land-forgectl-campaign
  ● cameron-batch-deployment
smart-home
  ○ hermes-egress-proxy-rules
```

Grouping selects `grouped_rows` in place of `rows`; it never edits the rows you
configured. Give `grouped_rows` the layout you want under a header — typically
without a `workspace` token, since the header already carries that name. `grouped_rows` takes precedence over
`rows_by_agent` while grouping is active; agent-specific overrides apply when
grouping is off.

Grouping applies only while the Agent panel is in **grouped** (space) order. It
turns off under **priority** order and under any agent view whose order
interleaves workspaces, because either would leave a header labeling agents from
somewhere else. A view that only filters keeps space order and stays grouped.
When it is off, the panel renders exactly as `group_by = "none"`
does. Scrolling into the middle of a run scrolls past that run's header; Herdr
does not pin headers.

#### Grouping Agent rows by a metadata token

Set `group_by = { token = "project" }` to group agents by a metadata token that
their panes report, such as `herdr pane report-metadata --token project=homelab`.
Agents that share a value share one header showing that value, even when they sit
in different workspaces. When the pane has no value, Herdr uses the same token
reported on the agent's workspace (`herdr workspace report-metadata`). An agent
with neither keeps its workspace group. The name may be written with or without the leading
`$`.

```toml
[ui.sidebar.agents]
group_by = { token = "project" }
grouped_rows = [
  ["state_icon", "$thread", "workspace"],
]
```

Members of one group need not be adjacent in space order. Herdr moves each
group's agents up behind the group's first agent, keeping their order, so the
drawn order, mouse targets, and next/previous agent navigation all agree. Under
an agent view that filters and sorts, every agent sharing a value stays in one
group in the view's order. Token grouping still turns off under **priority**
order. Pane tokens can expire (`--ttl-ms`); an agent whose token expired
uses its workspace's token, or returns to its workspace group.

Set `blocked_first = true` to move blocked agents to the top of their group, or
of the whole list when the panel is not grouped. Every other agent keeps its
place, so groups stay whole. It applies in **grouped** (space) order, including
under an agent view; **priority** order already puts blocked agents first.
...
An override replaces `rows`; it does not extend it. Override keys are case-sensitive canonical agent IDs such as `claude`, `codex`, and `pi`. Detection aliases such as `claude-code` are not accepted. Agents without an override, including custom-reported agents, use `rows`. While `group_by` is grouping entries, every agent uses `grouped_rows`, overrides included.
...
A plugin action whose manifest sets `destructive = true` is tagged in the
command palette and asks before it runs. For a third-party plugin whose author
has not set it, mark the action yourself:
...
Each entry is `"<plugin_id>:<action_id>"`, compared exactly and
case-sensitively against the joined pair — so it must match the ids as the
manifest declares them. An action is confirmed when either its manifest or this
list says so. Changes take effect on `herdr server reload-config`.

Both ids may themselves contain a colon, so two different pairs can join to the
same string. That collision asks about both, which costs an extra confirm and
never skips one.
