...
turns off under **priority** order and under any agent view that sorts, because
either can interleave workspaces and leave a header labeling agents from
somewhere else. When it is off, the panel renders exactly as `group_by = "none"`
