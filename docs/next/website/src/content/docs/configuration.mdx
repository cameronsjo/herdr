...
Each entry is `"<plugin_id>:<action_id>"`, matched as a whole pair — marking
one action never marks a sibling. An action is confirmed when either its
manifest or this list says so. Changes take effect on `herdr server
reload-config`.
