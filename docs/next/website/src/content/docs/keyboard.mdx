...
Press `prefix+/` to open the command palette. Before you type, it shows up to 12 useful actions: recently selected commands first, followed by a compact set of common workspace, tab, pane, and settings actions. Herdr remembers the eight most recent distinct commands across restarts. Arrow keys move the selection, `enter` runs the selected command, and `esc` closes. Clicking a row runs it too.

Start typing to search every available command. Matches are ranked by how well they answer what you typed, so a query naming one command exactly wins over a longer command that merely contains it. Plugin actions and panes include their kind when they would otherwise have identical names.
...
Two more move the focused tab the same way: `move tab to space` and `move tab to new space`. The tab moves whole, keeping its panes and their layout.
...
`move tab to space` opens the same picker, but a tab lands in a space rather than inside another tab, so only spaces and `+ new space` are offered. A space must keep at least one tab, so moving its last one is refused and a toast says so.
