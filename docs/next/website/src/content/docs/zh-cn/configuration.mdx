...
只有当 Agent 面板处于 **grouped**（空间）顺序时才会分组。在 **priority** 顺序以及任何会排序的智能体视图下，分组会关闭，因为两者都可能交错不同工作区，使标题标注到其他工作区的智能体。关闭时的渲染与 `group_by = "none"` 完全一致。滚动到一组的中间时，该组的标题也会随之滚出视图；Herdr 不会固定标题。
