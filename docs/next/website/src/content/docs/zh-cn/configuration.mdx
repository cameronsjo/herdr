...
`row_gap` 分别控制 Agent 和 Space 面板中条目之间的空白终端行数。默认值为 `0`，会紧密排列条目；设为 `1` 可恢复之前的间距。它不会在 `rows` 声明的内容行之间添加间距。连续缩进的 worktree 子项仍会作为一个 Space 组紧密排列。当 `group_by` 对 Agent 面板分组时，`row_gap` 只作用于工作区分组之间；同一组内的智能体会紧密排列在共享标题下。

#### 按工作区分组 Agent 行

设置 `group_by = "workspace"` 后，Herdr 会在每一组相邻的同工作区智能体上方绘制一个工作区标题，而不是在每个 Agent 条目上重复工作区名称：
...
分组只是用 `grouped_rows` 替换 `rows`，它不会修改你配置的行。由于标题已经展示了工作区名称，`grouped_rows` 通常不包含 `workspace` token。分组启用时，`grouped_rows` 优先于 `rows_by_agent`；按智能体的覆盖仅在分组关闭时生效。

只有当 Agent 面板处于 **grouped**（空间）顺序时才会分组。在 **priority** 顺序以及任何顺序会交错工作区的智能体视图下，分组会关闭，否则标题会标注到其他工作区的智能体。仅做过滤的视图保持空间顺序，因此仍会分组。关闭时的渲染与 `group_by = "none"` 完全一致。滚动到一组的中间时，该组的标题也会随之滚出视图；Herdr 不会固定标题。
...
覆盖项会替换 `rows`，而不是扩展它。覆盖项的键是区分大小写的规范智能体 ID，例如 `claude`、`codex` 和 `pi`。不接受 `claude-code` 等检测别名。没有覆盖项的智能体（包括自定义报告的智能体）会使用 `rows`；当 `group_by` 对条目分组时，所有智能体（包括有覆盖项的）都使用 `grouped_rows`。
