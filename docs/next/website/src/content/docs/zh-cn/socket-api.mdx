...
| 智能体 | `agent.list`、`agent.get`、`agent.read`、`agent.explain`、`agent.send_keys`、`agent.type_submit`、`agent.prompt`、`agent.wait`、`agent.rename`、`agent.focus`、`agent.start` |
...
策略,但不会重写配置。由于自定义排序可能交错不同工作区,视图生效期间还会暂停
`ui.sidebar.agents.group_by` 的工作区分组。
...
标签页事件订阅包括 `tab.created`、`tab.closed`、`tab.focused`、`tab.renamed`、`tab.moved` 和 `tab.moved_across_workspaces`。`tab.moved` 包含被移动的 `tab_id`、`workspace_id`、请求的 `insert_index` 和该工作区更新后的有序 `tabs` 列表。当标签页跨工作区移动时，改为触发 `tab.moved_across_workspaces`，其中包含重新签发的 `tab_id`、它在源工作区中的 `previous_tab_id`、`workspace_id`、`source_workspace_id`、`insert_index`，以及两个工作区的有序 `tabs` 和 `source_tabs` 列表。
