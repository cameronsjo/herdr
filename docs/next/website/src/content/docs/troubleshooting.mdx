...
## `herdr agent list` says no server is running but my panes are live

A herdr server has two sockets: the api socket used by the CLI, and a paired client socket used by the panes attached to it. If the server's api accept loop dies while the rest of the server keeps running, the CLI reports one of two new codes instead of the generic "no server is running" message:

- `server_api_not_accepting` — the api socket exists and refuses connections, but the client socket is still answering, so a server is definitely still up.
- `server_api_not_responding` — the api socket accepted the connection, but no handshake reply arrived.

**Do not run `herdr` in this state.** It would delete the refused socket file and start a second server, leaving the original one (and any panes attached to it) orphaned. Instead, check `herdr status server` and restart the stuck server deliberately:
