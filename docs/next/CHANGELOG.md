...
- Added palette commands to move the focused tab to an existing space or a new space. The tab moves whole, keeping its panes and their layout, and `move tab to space` opens the session navigator as a destination picker listing only spaces. A move that would leave its space with no tabs is refused rather than closing the space.
