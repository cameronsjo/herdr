...
- **The command palette (`src/ui/palette.rs`) sources its command list from the keybind-help entries (`src/ui/keybind_help.rs`), plus plugin actions/panes.** A new pane/workspace/tab action becomes palette-searchable automatically by adding a `help_action`/`help_action_kw` entry there — no separate palette registration. Matching ranks an entry's own name first; `HelpEntry`/`PaletteCommand.keywords` carries cross-vocabulary synonyms (e.g. the pane context menu's "Split right"/"Split down" wording for the palette's "split vertical"/"split horizontal" actions) so a query matches by name or synonym, with keyword matches always ranked below any name match.
