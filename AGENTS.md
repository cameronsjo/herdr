...
- **The command palette (`src/ui/palette.rs`) sources its command list from the keybind-help entries (`src/ui/keybind_help.rs`), plus plugin actions/panes.** A new pane/workspace/tab action becomes palette-searchable by its own name automatically by adding a `help_action`/`help_action_kw` entry there — no separate palette registration. Matching ranks an entry's own name first; `HelpEntry`/`PaletteCommand.keywords` carries cross-vocabulary synonyms (e.g. the pane context menu's "Split right"/"Split down" wording for the palette's "split vertical"/"split horizontal" actions), always ranked below any name match. Keywords are hand-authored per entry, not inferred or synced across related actions — when adding an action alongside existing ones that share a synonym (e.g. another "new pane"-shaped action), copy the shared keyword into its list too; nothing enforces siblings staying in sync.
...
A sidebar geometry test must derive its body rect the way the renderer does — `agent_panel_body_rect(area, should_show_scrollbar(metrics))`, never a hardcoded `false`. Once the content overflows, the renderer gives a column to the scrollbar, so a test passing `false` asserts against a width the renderer never used; the failure surfaces as a trailing scrollbar glyph in an otherwise correct row, which reads as a render bug rather than a test bug. `workspace_list_body_rect` behaves the same way.
