    ...
    let workspaces = snapshot
        ...
        .map(|(workspace_index, (workspace, state))| {
            ...
            protocol::ClientShellWorkspace {
                ...
                // `branch_label()`, not `branch()`: this value is sent to the
                // client to be drawn, and git permits control and format
                // characters in a branch name. `branch()` is the raw identity
                // form and must not reach a display path.
                branch: state.branch_label().map(|label| label.into_owned()),
