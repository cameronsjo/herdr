...
fn navigator_uses_machine_parents_only_for_federated_clients() {
    ...
    assert!(rows.iter().all(|row| match row.target {
        ClientNavigatorTarget::NewWorkspace => false,
    ...
    assert!(rows.iter().all(|row| match row.target {
        ...
        ClientNavigatorTarget::Machine { .. } | ClientNavigatorTarget::NewWorkspace => false,
    }));
}
...
#[test]
fn move_and_merge_pickers_only_offer_destinations_on_the_active_machine() {
    let (mut state, remote) = state_with_remote();
    for remote_active in [false, true] {
        if remote_active {
            assert!(state.activate_endpoint_projection(&remote));
        }
        for kind in 0..3 {
            ...
            assert_eq!(
                rows.iter()
                    .any(|row| row.target == ClientNavigatorTarget::NewWorkspace),
                kind != 2
            );
            assert!(rows.iter().all(|row| match &row.target {
                ClientNavigatorTarget::Workspace { endpoint_id, .. } =>
                    endpoint_id == &state.active_endpoint_id && row.depth == 0,
                ClientNavigatorTarget::Pane { endpoint_id, .. }
                | ClientNavigatorTarget::Tab { endpoint_id, .. } =>
                    kind == 0 && endpoint_id == &state.active_endpoint_id,
                ClientNavigatorTarget::NewWorkspace => kind != 2,
            ...
            if kind < 2 {
                assert!(outcome.actions.iter().any(|action| matches!(action,
                    ClientShellAction::Endpoint { endpoint_id, .. } if endpoint_id == &state.active_endpoint_id
                )));
                assert!(!outcome
                    .actions
                    .iter()
...
#[test]
fn switching_machines_dismisses_armed_fork_overlays() {
    let (mut state, remote) = state_with_remote();
...
#[test]
fn grouped_agents_keep_headers_and_gaps_separate_for_identical_workspace_ids() {
    use crate::api::schema::AgentStatus;
    let (mut state, remote_id) = state_with_remote();
    state.config.agents.group_by = crate::config::AgentGroupBy::Workspace;
    state.config.agents.row_gap = 1;
    let mut local = snapshot();
    local.workspaces[0].label = "local-group".into();
    local.agents = vec![
        agent("first", AgentStatus::Idle, 1),
        agent("second", AgentStatus::Idle, 1),
    ];
    local.agents[1].pane_id = "pane_2".into();
    state.set_snapshot(Box::new(local));
    let mut remote = snapshot();
    remote.boot_id = "remote-boot".into();
    remote.workspaces[0].label = "remote-group".into();
    remote.agents = vec![agent("third", AgentStatus::Idle, 1)];
    state.set_endpoint_snapshot(&remote_id, Box::new(remote));
    let frame = state.compose(100, 40).unwrap();
    let hits = &state.hits.endpoint_agents;
    assert_eq!(hits.len(), 3);
    assert_eq!(hits[0].0.height, 2);
    assert_eq!(hits[1].0.height, 1);
    assert_eq!(hits[2].0.height, 2);
    assert_eq!(hits[1].0.y, hits[0].0.bottom());
    assert_eq!(hits[2].0.y, hits[1].0.bottom() + 1);
    assert_eq!(hits[0].1, ClientEndpointId::Local);
    assert_eq!(hits[2].1, remote_id);
    let buffer = frame.to_ratatui_buffer().unwrap();
    for (hit, expected) in [(&hits[0], "local-group"), (&hits[2], "remote-group")] {
...
#[test]
fn aggregate_dragging_respects_endpoint_ownership_and_release_position() {
    use crate::api::schema::{AgentStatus, Method};
    fn mouse(state: &mut ClientShellState, kind: MouseEventKind, rect: Rect) -> ClientShellInput {
    ...
    local.agents = vec![agent("local", AgentStatus::Idle, 1)];
    ...
    let local_target = state
        ...
        .workspaces
        .iter()
        .find(|hit| hit.endpoint_id == ClientEndpointId::Local && hit.workspace_id == "ws_2")
        .unwrap()
        .rect;
    let foreign_target = state
        .hits
        .workspaces
        .iter()
        .find(|hit| hit.endpoint_id == remote_id && hit.workspace_id == "ws_2")
        .unwrap()
        .rect;
    let tab = state.hits.tabs[0].0;
    for first_target in [foreign_target, local_target] {
        ...
        assert!(!released.actions.iter().any(|action| matches!(action, ClientShellAction::Endpoint { request, .. } if matches!(request.method, Method::TabMove(_) | Method::TabMoveToDestination(_) | Method::PaneMove(_)))));
    }
    let local_agent = state
        .hits
        .endpoint_agents
        .iter()
        .find(|(_, id, _)| *id == ClientEndpointId::Local)
        .unwrap()
        .0;
    let pressed = mouse(
        &mut state,
    ...
    assert!(
        ...
        "focus waits so the press can become a drag"
    );
    assert!(state.agent_press.is_some());
    mouse(
        &mut state,
        MouseEventKind::Drag(MouseButton::Left),
        local_target,
    ...
    assert!(released.actions.iter().any(|action| matches!(action, ClientShellAction::Endpoint { endpoint_id: ClientEndpointId::Local, request, .. } if matches!(request.method, Method::PaneMove(_)))));
    let foreign_agent = state
        .hits
        .endpoint_agents
        .iter()
        .find(|(_, id, _)| *id == remote_id)
        .unwrap()
        .0;
    let activated = mouse(
        &mut state,
    ...
    assert!(activated.actions.iter().any(|action| matches!(action, ClientShellAction::ActivateEndpoint { endpoint_id, .. } if *endpoint_id == remote_id)));
    assert!(state.agent_press.is_none());
}
