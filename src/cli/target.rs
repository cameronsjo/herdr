...
pub(super) fn server_status(
    ...
    let probe = || {
        if is_remote() {
            ...
            // Bounded, not `client.status()`: a wedged server whose api socket
            // accepts but never replies must surface as a timeout so the
            // caller can report `server_api_not_responding` instead of hanging.
            client.status_with_timeout(super::HANDSHAKE_PROBE_TIMEOUT)
        }
    };
