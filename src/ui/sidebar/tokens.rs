...
pub(crate) fn agent_rows(
    ...
    config
        .rows_for_agent(context.canonical_agent, grouped)
        .iter()
        .filter_map(|row| {
            let resolved = row
                .iter()
...
"##).unwrap();
        ...
        for (machine, color, bold, dim) in [
            ...
            let rows = agent_rows(&config, context, "working", false);
        ...
        assert_eq!(
            agent_rows(&config, context(&entry), "working", false)[0].len(),
            1
        );
    }
...
"##).unwrap();
    ...
    #[test]
    fn matching_color_rule_preserves_right_alignment() {
        use ratatui::style::{Color, Style};
        let config: AgentsSidebarConfig = toml::from_str(
            r##"
rows = [[{ token = "workspace", align = "right", rules = [{ equals = "right", fg = "#f00" }] }]]
"##,
        )
        .unwrap();
        let mut entry = entry();
        entry.workspace = "right".into();
        let rows = agent_rows(&config, context(&entry), "idle", false);
        assert_eq!(
            rows[0][0].style.align,
            Some(crate::config::SidebarTokenAlignment::Right)
        );
        ...
        assert_eq!(spans.last().unwrap().style.fg, Some(Color::Rgb(255, 0, 0)));
    }
...
"#,
        ...
        for (value, bold, dim) in [
            ...
            let rows = agent_rows(&config.agents, context(&entry), "working", false);
    ...
    fn machine_token_only_resolves_for_multi_machine_rows() {
        ...
        assert_eq!(
            agent_rows(&config, context(&entry), "working", false),
        ...
        assert_eq!(
            agent_rows(&config, remote_context, "working", false),
            vec![vec![
    ...
    fn state_text_and_arbitrary_values_are_independent_tokens() {
        ...
        assert_eq!(
            agent_rows(&config, context(&entry), "deep in the mines", false),
            vec![vec![
    ...
    fn known_agent_override_replaces_default_rows() {
        ...
        assert_eq!(
            agent_rows(&config, context(&pi), "working", false),
